...
## 済ませるためで、20m 四方のプリミティブ構成なら同期ベイクでも一瞬で終わる。
## 対象ジオメトリは `nav_source` グループ（床・壁・柱）から集める。什器が増えたら
## そのノードを同じグループに入れる。銀行ロビー（8/18〜）では事前ベイク済みの
## NavigationMesh リソースへ移す想定。
...
func _bake_navigation() -> void:
	...
	if region == null:
		push_warning("test_stage: NavigationRegion3D が見つからない")
		return
	...
	print("[test_stage] navmesh baked: polygons=%d vertices=%d" %
		[nav_mesh.get_polygon_count(), nav_mesh.vertices.size()])
