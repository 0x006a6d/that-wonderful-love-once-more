...
- ファイル: `assets/motions/mixamo_*.fbx` (計 38 本)
  ...
  - キック・ひざ (7): `mixamo_kick_soccer` (Kick Soccerball) / `mixamo_knee` (Illegal Knee) / `mixamo_kick_finish` (Kicking) / `mixamo_knee_jab` (Knee Jab) / `mixamo_kick_chapa` (Chapa-Giratoria) / `mixamo_kick_martelo` (Martelo 2) / `mixamo_kick_standing` (Standing Melee Kick)
